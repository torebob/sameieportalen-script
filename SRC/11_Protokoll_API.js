...
(() => {
  const _PG_HEADERS_ = globalThis.SHEET_HEADERS?.[SHEETS.PROTOKOLL_GODKJENNING] ||
  ...
  const _hdrIdxMap_ = (headers, names) => names.reduce((acc, name) => {
      acc[name] = headers.indexOf(name);
      return acc;
    }, {});

  const _uuid8_ = () => Utilities.getUuid().replace(/-/g, '').slice(0, 8);
  ...
  const _findMoteRow_ = (moteId) => {
    const ss = SpreadsheetApp.getActive();
    const sh = ss.getSheetByName(SHEETS.MOTER);
    if (!sh || sh.getLastRow() < 2) return null;

    const headers = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0];
    const H = _hdrIdxMap_(headers, _MOTE_HEADERS_FALLBACK_);
  ...
  const _ensureSheetLocal_ = (name, headers) => {
    const ss = SpreadsheetApp.getActive();
  ...
  globalThis.sendProtokollForGodkjenning = (moteId, protokollUrl) => {
    try {
      ...
      const mote = _findMoteRow_(moteId);
  ...
  globalThis.handleProtokollApprovalRequest = (e) => {
    ...
    try {
      ...
      const mote = moteId ? _findMoteRow_(moteId) : null;
